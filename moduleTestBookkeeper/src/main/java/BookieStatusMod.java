 ...
 */
public class BookieStatusMod {

    private static final Logger LOG = LoggerFactory.getLogger(BookieStatusMod.class);
    ...
    void readFromDirectories(List<File> directories) {
        ...
        for (File dir : directories) {
            File statusFile = new File(dir, BOOKIE_STATUS_FILENAME);
            try {
                BookieStatusMod status = readFromFile(statusFile);
                if (null != status) {
                    synchronized (status) {
     ...
     * @throws IOException
     */
    private BookieStatusMod readFromFile(File file)
     ...
     * @throws IOException
     */
    public BookieStatusMod parse(BufferedReader reader)
            throws IOException, IllegalArgumentException {
        BookieStatusMod status = new BookieStatusMod();
